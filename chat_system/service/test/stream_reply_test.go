...
func TestStreamReply_StreamToolCallsRemainComplete(t *testing.T) {
	...
	if toolResultEvent["id"] != "call_1" || toolResultEvent["tool"] != "unknown_tool" {
		t.Fatalf("unexpected tool_result event: %+v", toolResultEvent)
	}
	if _, exists := toolResultEvent["result"]; exists {
		t.Fatalf("tool_result event should not expose detailed result: %+v", toolResultEvent)
	}
}
