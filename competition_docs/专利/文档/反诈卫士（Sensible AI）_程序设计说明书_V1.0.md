...
【插图占位：图 1 系统总体架构图】  
【图注建议：展示终端层、后端服务层、主业务数据库、案件知识库数据库、Redis、模型服务和实时告警链路之间的关系。】
...
【插图占位：图 2 系统功能结构图】  
【图注建议：展示用户端、管理员端和后端服务模块之间的功能树关系。】
...
【插图占位：图 3 普通用户完整分析流程图】  
【图注建议：展示从用户提交内容到结构化报告输出、历史归档和通知触发的流程。】
...
【插图占位：图 4 快路径识别流程图】  
【图注建议：展示悬浮球触发、无障碍监听、OCR 兜底、关键词加权评分、分级提醒和进入慢路径的流程。】
...
【插图占位：图 5 后端模块架构图】  
【图注建议：展示 `cmd/api`、`internal/bootstrap`、`internal/platform`、`internal/modules` 之间的关系。】
...
【插图占位：图 6 Android 端模块架构图】  
【图注建议：展示 UI 层、后台服务层、无障碍守护层、OCR 与快判层、网络调用层和通知层之间的关系。】
...
【插图占位：图 7 多智能体协同流程图】  
【图注建议：展示子智能体并发取证、主智能体工具闭环和历史归档之间的关系。】
...
【插图占位：图 8 数据库 ER 图】  
【图注建议：展示 users、history_cases、user_history_vectors、family 系列表、historical_case_library 和 pending_review_cases 之间的关系。】
...
1. 文本和图片分析平均响应时间:70.18s(10份文本和10份图文混合分析平均时长)
2. 音频和视频分析平均耗时:120.65s(5份音频和5份视频分析平均时长)
3. 多任务并发处理稳定性:10个并发任务平均响应时间:90.32s(10个并发任务平均响应时间)
4. Redis 缓存命中与回退逻辑:数据库缓存命中率100%(100次缓存命中)

### 14.3 稳定性测试

1. 长时间后台守护场景:持续运行24小时，无崩溃、无内存泄漏、无功能异常。 
2. WebSocket 断线重连:持续心跳100次，无断线、无异常。  
3. 模型调用失败后的容错:100次模型调用，无异常。  
4. 缓存异常后的功能降级:64次缓存异常降级数据库取值，缓存自动修复，无明显异常
5. 历史归档与家庭通知的链路完整性:全部测试案例均正常

### 14.4 快路径专项测试

对于 Android 主动守护快路径，建议专项测试以下场景：

1. 无障碍权限开启时的节点文本识别与加权评分。  
2. 无障碍权限关闭时的截图 OCR 与加权评分。  
3. 强风险词、中风险词和组合词的阈值触发效果。  
4. 页面文本为空、OCR 失败或文本噪声较大时的降级处理。  
5. 提醒与深度分析转入逻辑是否正常。  
