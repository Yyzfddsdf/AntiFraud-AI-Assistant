# 《反诈卫士（Sentinel AI）》程序设计说明书

软件名称：反诈卫士（Sentinel AI）  
版本号：V1.0  
文档类型：程序设计说明书  
...
《反诈卫士（Sentinel AI）》是一套面向普通公众的智能反诈软件系统。系统围绕“风险识别、结果解释、历史沉淀、主动提醒、家庭联防、知识治理”六条主线组织功能，旨在帮助用户在遭遇可疑文本、图片、音频、视频或高风险页面时，快速获得结构化风险分析结果，并通过历史记忆、动态阈值和联防机制形成持续性防护能力。
...
9. AI 反诈模拟训练子系统。  
10. 终端交互层设计，包括管理员端、移动端及 Android 端主动守护能力设计。  
...
1. 用户交互层。  
   ...
   2. 移动端采用独立移动前端工程，并对应 Android 原生端设计，用于分析提交、聊天咨询、风险历史、趋势查看、家庭中心、主动守护和模拟训练。  
...
8. Android 原生端设计负责无障碍守护、悬浮球与后台提醒。  
...
3. Android 原生端设计方案。  
   Android 原生端主要负责后台守护、悬浮球、无障碍监听、OCR 兜底、通知提醒和家庭联动入口等高频触发能力。  
...
为了便于维护和排查，核心表与业务模块的对应关系如下：

1. `users` 对应登录注册、身份认证和用户画像基础信息。  
2. `family_groups`、`family_members`、`family_invitations`、`family_guardian_links`、`family_notifications` 对应家庭中心与家庭联防。  
3. `pending_tasks` 对应多模态分析任务的进行中状态。  
4. `history_cases` 对应用户历史归档与风险回看。  
5. `user_history_vectors` 对应用户历史语义记忆。  
6. `simulation_quiz_packs`、`simulation_quiz_sessions`、`simulation_quiz_generation_tasks` 对应 AI 反诈模拟训练。  
7. `historical_case_library`、`pending_review_cases` 对应历史案件知识库与待审核队列。  

### 8.7 数据设计补充说明

1. 主业务库与知识库分离，避免案例持续扩展影响登录、历史和家庭等稳定业务。  
2. 用户历史向量与正式知识库向量分开维护，便于分别做个人召回和全局召回。  
3. 任务表与历史表分离，便于把“处理中”和“已归档”两类状态区分清楚。  
4. 审核前后分库或分表管理，避免未经审核的案件直接影响正式知识库。  
...
1. `GET /api/auth/captcha` 用于获取图形验证码。  
2. `POST /api/auth/sms-code` 用于获取短信验证码。  
3. `POST /api/auth/register` 用于注册账号。  
4. `POST /api/auth/login` 用于登录并获取 JWT。  
5. `GET /api/user` 用于获取当前用户基础信息。  
6. `PUT /api/user/profile` 用于更新个人资料。  

#### 10.2.2 地区与风险信息接口

1. `GET /api/regions/provinces` 用于获取省份列表。  
2. `GET /api/regions/cities` 用于获取城市列表。  
3. `GET /api/regions/districts` 用于获取区县列表。  
4. `POST /api/regions/resolve` 用于解析地区编码。  
5. `GET /api/regions/cases/stats/current` 用于获取当前登录用户所在地区案件统计。  

#### 10.2.3 多模态分析与历史接口

1. `POST /api/scam/image/quick-analyze` 用于图片快判。  
2. `POST /api/scam/multimodal/analyze` 用于多模态综合分析。  
3. `GET /api/scam/multimodal/tasks` 用于查看任务状态。  
4. `GET /api/scam/multimodal/tasks/:taskId` 用于查看任务详情。  
5. `GET /api/scam/multimodal/history` 用于查看历史记录。  
6. `GET /api/scam/multimodal/history/overview` 用于查看风险总览。  
7. `DELETE /api/scam/multimodal/history/:recordId` 用于删除历史记录。  

#### 10.2.4 聊天、告警与家庭接口

1. `POST /api/chat` 用于发起聊天式反诈咨询。  
2. `GET /api/chat/context` 用于查看当前聊天上下文。  
3. `POST /api/chat/refresh` 用于清空聊天上下文。  
4. `GET /api/alert/ws` 用于接收主动告警。  
5. 家庭相关接口用于创建家庭、加入家庭、配置守护关系和接收家庭通知。  
6. 模拟训练相关接口用于生成题包、答题、查看题包和查看训练结果。  

#### 10.2.5 用户端接口返回说明

1. 成功时通常返回可直接渲染的数据对象。  
2. 请求参数缺失或格式不对时返回可读错误信息。  
3. JWT 失效时返回 `401`，前端应引导重新登录。  
4. 权限不足时返回 `403`，前端不应继续重试。  
5. 多模态分析一类长任务会先返回任务状态，再由任务详情接口补全最终结果。  
...
1. `POST /api/scam/case-library/cases` 用于上传历史案件并触发向量化入库。  
2. `GET /api/scam/case-library/cases` 用于查看案件列表。  
3. `GET /api/scam/case-library/cases/:caseId` 用于查看案件详情。  
4. `DELETE /api/scam/case-library/cases/:caseId` 用于删除正式案例。  
5. `GET /api/scam/case-library/cases/overview` 用于查看案件总体统计。  
6. `GET /api/scam/case-library/cases/graph` 用于查看案件关系图。  
7. `GET /api/scam/case-library/cases/geo-map` 用于查看地区态势数据。  
8. `GET /api/scam/case-library/maps/geojson` 用于获取地图地理边界数据。  
9. `GET /api/scam/case-library/options/scam-types` 用于获取诈骗类型选项。  
10. `GET /api/scam/case-library/options/target-groups` 用于获取目标人群选项。  
11. `POST /api/scam/case-collection/search` 用于案情检索。  

#### 10.3.2 审核接口

1. `GET /api/scam/review/cases` 用于查看待审核案件。  
2. `GET /api/scam/review/cases/:recordId` 用于查看待审核案件详情。  
3. `POST /api/scam/review/cases/:recordId/approve` 用于审核通过。  
4. `POST /api/scam/review/cases/:recordId/reject` 用于审核驳回。  

#### 10.3.3 管理端接口使用边界

1. 管理端接口仅限管理员调用。  
2. 管理端接口不应混入用户端页面逻辑。  
3. 审核未通过的案件应停留在待审核层，不应直接写入正式知识库。  
4. 地图、图谱和统计接口以查询为主，不应反向修改主业务数据。  
...
1. 接口响应应尽量保持字段命名稳定，避免前端多分支适配。  
2. 长任务接口优先返回任务状态，再由详情接口返回完整内容。  
3. 对于家庭通知、聊天和分析结果这类高频交互接口，错误提示应尽量直接可读。  
4. 如果外部依赖失败，响应中应保留可追踪上下文，但不暴露敏感密钥。  
...
1. 图形验证码过期时，登录接口应提示刷新验证码，而不是直接返回模糊失败。  
2. Redis 临时不可用时，聊天上下文、地区统计和部分缓存查询应尽量降级，不应拖垮主服务。  
3. 向量库缓存不一致时，应优先以数据库为准进行修复，而不是直接返回空结果。  
4. 多模态子任务失败时，主流程应保留失败原因，保证用户至少能看到可读的中间状态。  
5. 家庭通知失败时，不应影响历史归档和风险结果保存。  
...
2. **识别准确率 (Accuracy)**：整体风险定性准确率**大于 95%**，整体识别结果具有较高稳定性和可用性。
...
3. **极端异常降级**：
   ...
   - 模拟 **64 次 Redis 异常**：系统自动切换数据库取值并伴随自动重连修复，相关异常场景下系统能够完成自动恢复，测试中恢复成功率为 100%。

### 14.5 测试总结

**《反诈卫士》系统经过严格的功能验证、性能压测及 AI 精度专项对比，各项指标均已满足“高可用、高精度、低干扰”的设计目标。系统具备成熟的异常处理边界与较低的模块间耦合度，测试结果表明，系统整体已达到预期设计目标，可支持后续软件著作权登记材料整理、系统归档与部署参考。**
...
《反诈卫士（Sentinel AI）》通过多模态分析、双重 RAG、动态阈值、快慢双路径、家庭联防、知识治理和模拟训练等机制，构建了一套具备完整闭环的软件系统。其程序结构清晰、模块边界明确、数据流可追踪、关键能力具备稳定落地基础。
