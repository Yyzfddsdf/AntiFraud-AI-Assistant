...
本文档中的界面截图可根据最终提交版本统一补充。

由于当前需要先完成完整文字稿，文中所有图片均先以占位形式标注。

为便于后续整理，本稿先以截图位置说明代替正式插图。

本文统一使用以下格式：

> 图片占位：填写此处应插入的页面截图名称  
> 建议图片内容：说明截图中应包含的按钮、卡片、弹窗、结果区或高亮标注  
> 建议标注方式：作为操作说明，可在图上用数字 1、2、3 标注重点区域
...
> 图片占位：首次打开应用后的欢迎页或登录页全屏截图  
> 建议图片内容：显示应用 Logo、登录入口、注册入口、协议提示、验证码区域  
> 建议标注方式：标出“登录”“注册”“验证码”“提交按钮”四个位置
...
> 图片占位：登录页结构标注图  
> 建议图片内容：输入框、登录方式切换、图形验证码图片、提交按钮  
> 建议标注方式：用数字编号说明每个输入项作用
...
> 图片占位：首页整体布局截图  
> 建议图片内容：顶部欢迎文案、守护状态、历史档案、趋势图、快捷入口  
> 建议标注方式：标明“首页核心信息分布”
...
> 图片占位：文本分析页截图  
> 建议图片内容：大文本输入框、上传附件区域、开始检测按钮  
> 建议标注方式：标出“输入文字”“上传附件”“开始全面检测”
...
1. 只输入“这个是不是诈骗”而不提供上下文。
2. 只贴一个链接，不说明对方要求做什么。
3. 删掉了关键金额、验证码、身份冒充信息。
4. 只贴自己的总结，没有贴对方原话。
...
> 图片占位：图片分析页截图  
> 建议图片内容：上传图片区、已选图片缩略图、开始分析按钮  
> 建议标注方式：标出“选择图片”“已上传成功”“开始检测”
...
2. 不要过度裁剪。
3. 避免模糊。
4. 金额、时间、联系人、账号、按钮文字尽量可见。
5. 如果是长聊天，建议连续截取几张关键图。
...
> 图片占位：音频上传与分析页截图  
> 建议图片内容：音频文件选择区域、已选文件状态、分析按钮、处理中状态  
> 建议标注方式：标明“音频文件已就绪”“分析中”“结果入口”
...
> 图片占位：视频分析任务页截图  
> 建议图片内容：视频缩略图、任务状态、处理中提示、完成后详情入口  
> 建议标注方式：标出“视频已上传”“任务处理中”“查看结果”
...
深度综合分析是系统在用户提交较完整证据后，对文本、图片、音频、视频等线索进行综合判断的过程。

它更适合以下情况：

1. 您已经掌握较多证据。
2. 您希望得到更完整的风险报告。
3. 您不仅想知道“高不高”，还想知道“为什么”。
4. 您希望得到更明确的处理建议。
...
> 图片占位：分析结果详情页完整截图  
> 建议图片内容：风险等级标签、类型标签、综合分析报告、建议动作  
> 建议标注方式：标出“先看哪里”“再看哪里”“建议立即执行哪里”
...
> 图片占位：消息中心列表页截图  
> 建议图片内容：消息卡片、风险等级标签、时间信息、点击进入详情  
> 建议标注方式：说明“未读消息”“高风险消息”“查看详情”
...
> 图片占位：历史记录列表截图  
> 建议图片内容：多条历史卡片、风险等级标签、时间信息、详情按钮  
> 建议标注方式：说明“最近记录”“高风险记录”“点开查看”
...
> 图片占位：风险趋势分析页面截图  
> 建议图片内容：趋势图、摘要卡片、时间范围切换、统计卡片  
> 建议标注方式：说明“近期变化”“高风险占比”“系统摘要”
...
> 图片占位：地区风险信息页截图  
> 建议图片内容：今日、近7天、近30天统计切换，高发骗局列表  
> 建议标注方式：说明“时间范围”“Top 列表”“本地趋势”
...
> 图片占位：AI 助手聊天页截图  
> 建议图片内容：聊天气泡、输入框、发送按钮、清空对话按钮  
> 建议标注方式：说明“提问区”“助手回复区”“继续追问”
...
> 图片占位：通知权限开启引导页截图  
> 建议图片内容：权限说明、跳转按钮、开启后状态  
> 建议标注方式：说明“为什么要开”“去系统设置”“开启成功”
...
> 图片占位：悬浮窗权限开启页截图  
> 建议图片内容：开关、系统授权页、授权后的悬浮球显示效果  
> 建议标注方式：说明“开关位置”“系统授权”“悬浮球位置”
...
> 图片占位：无障碍守护开启流程截图  
> 建议图片内容：应用内引导页、系统无障碍设置页、开启成功状态  
> 建议标注方式：按步骤 1、2、3 标出关键操作
...
> 图片占位：家庭中心首页截图  
> 建议图片内容：家庭状态、邀请入口、成员列表、守护关系区  
> 建议标注方式：说明“创建家庭”“加入家庭”“管理入口”
...
> 图片占位：邀请与加入家庭流程截图  
> 建议图片内容：邀请码、加入入口、收到的邀请列表、加入成功提示  
> 建议标注方式：标出“谁发起”“谁加入”“成功后哪里看”
...
> 图片占位：守护关系配置页截图  
> 建议图片内容：守护人下拉选择、被守护成员下拉选择、保存按钮、已配置关系列表  
> 建议标注方式：说明“左边守护人”“右边被守护人”“下方已生效关系”
...
> 图片占位：家庭联防通知弹窗截图  
> 建议图片内容：家庭成员名、风险摘要、进入家庭中心按钮  
> 建议标注方式：突出“谁出风险”“风险类型”“下一步入口”
...
> 图片占位：模拟训练首页截图  
> 建议图片内容：题包列表、生成题包按钮、历史报告入口  
> 建议标注方式：说明“开始新训练”“继续未完成”“查看历史报告”
...
> 图片占位：我的页面截图  
> 建议图片内容：头像区、权限状态区、退出登录按钮、隐私入口  
> 建议标注方式：说明“权限检查入口”“账号入口”“退出入口”
...
建议根据实际发布版本补充界面截图并统一排版，以形成完整、清晰的正式用户操作说明书。
