...
import (
	...
	model "antifraud/internal/modules/multi_agent/adapters/outbound/case_library/model"
	"antifraud/internal/platform/cache"
	"antifraud/internal/platform/database"
)

const (
	scamTypesConfigPath    = "internal/platform/config/scam_types.json"
	targetGroupsConfigPath = "internal/platform/config/target_groups.json"
)
...
func resolveTargetGroupsConfigPath() string {
	...
	if ok {
		...
		for i := 0; i < 10; i++ {
			candidate := filepath.Join(currentDir, targetGroupsConfigPath)
...
func resolveScamTypesConfigPath() string {
	...
	if ok {
		currentDir := filepath.Dir(currentFile)
		for i := 0; i < 10; i++ {
			candidate := filepath.Join(currentDir, scamTypesConfigPath)
			if _, err := os.Stat(candidate); err == nil {
				return candidate
			}
			parent := filepath.Dir(currentDir)
