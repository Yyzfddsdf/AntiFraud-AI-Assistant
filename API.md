...
- 仅管理员可调用。
- 返回全国省级总览，不再携带省内城市和区县整棵树。
- 数据来源为：
  - 用户历史案件：`history_cases`
  - 用户位置：`users` 表中的 `province_code / city_code / district_code`
- 返回字段包含：
  - 全国摘要：定位用户数、历史案件数、省份数、城市数
  - 省级地区列表
  - 每个省在 `today / last_7d / last_30d / all_time` 下的聚合统计
  - 每个省 Top3 高发骗局、环比变化趋势、风险等级
- 城市与区县数据需通过懒加载接口 `/api/scam/case-library/cases/geo-map/children` 获取。
...
## 18.4) 查询指定父级地区的子级统计（仅管理员）

- **Method**: `GET`
- **Path**: `/api/scam/case-library/cases/geo-map/children?parent_code=<父级行政区编码>&level=city|district`
- **Header**:
  - `Authorization: Bearer <JWT_TOKEN>`
  - `Accept: application/json`
...
- 仅管理员可调用。
- 用于管理员地理态势页在省级总览基础上按需懒加载下一层级：
  - `level=city`：返回指定省份下的全部城市
  - `level=district`：返回指定城市下的全部区县
- `parent_code` 必填：
  - 当 `level=city` 时，传省级行政区编码
  - 当 `level=district` 时，传市级行政区编码
- 返回的每个子地区同样包含 `today / last_7d / last_30d / all_time` 聚合统计与 `top_scam_types`。

### 成功响应（200）

```json
{
  "generated_at": "2026-03-27T23:30:00+08:00",
  "level": "city",
  "parent_code": "330000",
  "parent_name": "浙江省",
  "region_count": 11,
...
```bash
curl -X GET "http://<HOST>/api/scam/case-library/cases/geo-map/children?parent_code=330000&level=city" \
  -H "Authorization: Bearer <JWT_TOKEN>"
```

---

## 18.5) 查询指定地区案件摘要分页列表（仅管理员）

- **Method**: `GET`
- **Path**: `/api/scam/case-library/cases/geo-map/region-cases?region_code=<行政区编码>&window=<时间窗口>&page=1&page_size=10`
- **Header**:
  - `Authorization: Bearer <JWT_TOKEN>`
  - `Accept: application/json`
...
- 仅管理员可调用。
- 用于管理员地理态势页查看某个省/市/区县在指定时间窗口下的案件摘要列表。
- `region_code` 必填，支持省级、市级、区县级行政区编码。
- `window` 可选，支持以下取值：
  - `today`
  - `last_7d`
  - `last_30d`
  - `all_time`
- `page` 可选，默认 `1`。
- `page_size` 可选，默认 `10`；当前接口按固定档位收敛为 `10 / 20 / 50`。

### 成功响应（200）

```json
{
  "generated_at": "2026-03-27T23:10:00+08:00",
  "region_code": "330000",
  "region_name": "浙江省",
  "window": "last_7d",
  "case_count": 18,
  ...
  "cases": [
    {
      "record_id": "TASK-20260327-001",
      "title": "冒充客服退款诈骗",
      "case_summary": "诈骗分子冒充电商客服，以商品质量问题退款为由诱导受害人共享屏幕并转账。",
      "scam_type": "冒充客服类",
      "risk_level": "高",
      "created_at": "2026-03-27T18:20:00+08:00"
    }
  ]
}
```
...
```bash
curl -X GET "http://<HOST>/api/scam/case-library/cases/geo-map/region-cases?region_code=330000&window=last_7d&page=1&page_size=10" \
  -H "Authorization: Bearer <JWT_TOKEN>"
```

---

## 18.6) 获取地图边界 GeoJSON（仅管理员）
