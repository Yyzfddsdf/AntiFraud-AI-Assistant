...
export function useChartsModule(deps) {
  ...
  const regionCaseStatsNeedsSetup = ref(false);
  const regionCaseStatsMessage = ref('');
  const riskCache = ref({});
  ...
  const fetchCurrentRegionCaseStats = async () => {
    if (!deps.isAuthenticated.value) {
      regionCaseStats.value = null;
      regionCaseStatsNeedsSetup.value = false;
      regionCaseStatsMessage.value = '';
      return;
    }
    try {
      const res = await deps.request('/regions/cases/stats/current', 'GET', null, { silent: true, throwOnError: true });
      if (res) {
      ...
      if (Number(error?.status) === 400) {
        regionCaseStatsNeedsSetup.value = true;
        regionCaseStatsMessage.value = String(error?.message || '请先在个人资料中设置所在地区后再查看。').trim();
        return;
      }
      regionCaseStatsNeedsSetup.value = false;
      regionCaseStatsMessage.value = '';
  ...
  return {
    ...
    riskData,
    regionCaseStats,
    regionCaseStatsNeedsSetup,
    regionCaseStatsMessage,
    adminStatsInterval,
    adminStatsData,
    adminGraphData,
