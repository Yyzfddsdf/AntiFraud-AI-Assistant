<template>
  <div class="w-full animate-fade-in">
    ...
    <div class="grid gap-6 2xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,0.75fr)]">
      <div class="space-y-6 min-w-0">
        <div v-if="riskData && riskData.analysis" class="bg-white p-6 rounded-sm shadow-sm border border-slate-200">
          <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 xl:min-w-[320px]">
      ...
      <div class="space-y-6 min-w-0">
        <div v-if="riskData && riskData.stats" class="bg-white p-6 rounded-sm shadow-sm border border-slate-200">
          <h3 class="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
            <span class="w-1.5 h-4 bg-slate-700 rounded-sm"></span>
            风险快照
          </h3>
          <div class="grid grid-cols-2 gap-3">
            <div class="rounded-sm border border-slate-200 bg-slate-50 px-4 py-3">
              <div class="text-[11px] font-bold uppercase tracking-[0.16em] text-slate-400">总案件</div>
              <div class="mt-1 text-2xl font-black text-slate-900">{{ riskData.stats.total || 0 }}</div>
            </div>
            <div class="rounded-sm border border-red-100 bg-red-50/70 px-4 py-3">
              <div class="text-[11px] font-bold uppercase tracking-[0.16em] text-red-400">高风险</div>
              <div class="mt-1 text-2xl font-black text-red-700">{{ riskData.stats.high || 0 }}</div>
            </div>
            <div class="rounded-sm border border-amber-100 bg-amber-50/70 px-4 py-3">
              <div class="text-[11px] font-bold uppercase tracking-[0.16em] text-amber-500">中风险</div>
              <div class="mt-1 text-2xl font-black text-amber-700">{{ riskData.stats.medium || 0 }}</div>
            </div>
            <div class="rounded-sm border border-emerald-100 bg-emerald-50/70 px-4 py-3">
              <div class="text-[11px] font-bold uppercase tracking-[0.16em] text-emerald-500">低风险</div>
              <div class="mt-1 text-2xl font-black text-emerald-700">{{ riskData.stats.low || 0 }}</div>
            </div>
          </div>
        </div>

        <div v-if="regionCaseStatsNeedsSetup" class="bg-amber-50 border border-amber-200 rounded-sm px-6 py-5 flex items-start justify-between gap-4">
          <div>
        ...
        <div v-else-if="regionCaseStats && regionCaseStats.region" class="bg-white p-6 rounded-sm shadow-sm border border-cyan-100">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 2xl:grid-cols-1 gap-4 mt-5">
